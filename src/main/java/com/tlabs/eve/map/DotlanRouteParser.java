...
public class DotlanRouteParser implements EveParser<DotlanRouteResponse> {
	...
	private static final String XSLT = "/com/tlabs/eve/map/dotlan-route.xsl";
	...
	private List<SolarSystem> parseImpl(String html) throws IOException {
		String replaced = 
				StringUtils.substringAfter(
						html, 
						"<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\" class=\"tablelist table-tooltip\">");
	...
	private static final String getChildText(final NodeList list, final int index) {
	...
	private static final Float getChildFloat(final NodeList list, final int index) {
		if (index >= list.getLength()) {
			return 0f;
		}
		String text = list.item(index).getTextContent();
	...
	private static final Integer getChildInt(final NodeList list, final int index) {
		if (index >= list.getLength()) {
			return 0;
		}
		String text = list.item(index).getTextContent();
