...
public abstract class EveXMLParser<T extends EveResponse> extends AbstractXMLParser<T> {
	...
	private static class ErrorRule extends BaseRule {
		public void doBegin(java.lang.String name, org.xml.sax.Attributes attributes) {
			...
			EveResponse r = (EveResponse)getDigester().peek();
			try {
	...
	private abstract static class DateRule extends BaseRule {
		...
		public abstract void setDate(EveResponse r, Date d);
		
	}
	
	public EveXMLParser(Class<T> responseClass) {
		super(responseClass);
	}
	...
	protected final void init(final Digester digester) {
		...
		digester.addRule("eveapi/currentTime", new DateRule() {
			public void setDate(EveResponse r, Date d) {
